...
//        } 
    ...
    //console.dir(jsondata);
      ...
      //console.log(segmentArr_[i].b,segmentArr_[i].e,segmentArr_[i].a);
		  ...
		  for(var j=segmentArr_[i].b;j<=segmentArr_[i].e;j++)
		  {
			  cumulitive+=segmentArr_[i].a*segmentArr_[i].a;
		  }
      ...
      basic.on('mouseover', function(){
       ...
       //console.log(itr,yMin[itr],yMax[itr]);
    ...
    // console.log(itr, ' outside of error',xMin);
	 doneFlag=true;
	 ...
	 if(itr==xMin)
		{
			...
			for(var i=0;i<segmentArr_.length;i++)
			{
				for(;j<=segmentArr_[i].e;j++)
					series1[j] = (1.0*segmentArr_[i].a)/cumulitive;
			}*/
		...
		else if(itr > xMin)
		{
			...
			{
				for(;j<=segmentArr_[i].e;j++)
					series2[j] = (1.0*segmentArr_[i].a)/cumulitive;
			}
			
			var ned=0;
			for(var i=xMin;i<=xMax;i++)
			{
				ned+= (series1[i]-series2[i])*(series1[i]-series2[i]);
		...
		//	console.log("NED",ned);
			...
			var rectangle1 = errorBar.append("rect")
                            .attr("x", 0)
                           .attr("y", 0)
                           .attr("width", ned*100)
						   .attr("height", 20)
						   .style("fill", "red")
						   .style("stroke", '#555');
		  rectangle1.append("title").text(ned);
		  var rectangle2 = errorBar.append("rect")
                            .attr("x", ned*100)
                           .attr("y", 0)
                           .attr("width", 100-ned*100)
						   .attr("height", 20)
						   .style("fill", "white")
						   .style("stroke", '#555');
...
function loadAxesColumns(){
  ...
  var info = {"dimension": dimension, "measure": measure, "predicate": filter, "predicatevalue":filterValue, "sampling":samplingType, "epsilon":8.0, "alpha":1.0};
