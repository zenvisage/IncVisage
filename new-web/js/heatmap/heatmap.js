...
function render(itr, width, height, viz, segmentArr_,estimateArr_){
    ...
    var legendElementHeight = 28;
	  ...
	  if(itr>=iterationLow)	  
	 {
		...
		//var legendElementWidth = Math.ceil(((1.0*(widthWin + margin.left + margin.right))/2)/(2*(avgMax[itr]-avgMin[itr])));
		//i=Math.ceil(avgMin[itr]),
    //Math.ceil(avgMax[itr])
    var numLegends = 50;
...
function loadAxesColumns(){
  ...
  var info = {"dimension1": dimension1,"dimension2":dimension2, "measure": measure, "predicate": filter, "predicatevalue":filterValue,"sampling":samplingType};
