...
$('#forward').click(function() {
	  ...
	  if(iteration<iterationLow)
	  {
		  ...
		  document.getElementById("play").style.opacity = "1";
		  document.getElementById("back").style.opacity = "1";
			$.get('/getiterativedata1', JSON.stringify(iteration), genChartsBase,'json')
			.fail(function() {
...
function playBase(){
	...
	if(resetFlag==false)
	{
		...
		console.log("base iteration:",iteration,"Max:",maxIteration,"isContinue:",isContinue);
		if(playFlag==true && isContinue==1)//iteration<=maxIteration  && playFlag==true
		{
...
function simulateplayBase()
{
	...
	{	 
		 ...
		 else
		 {
		  	...
		  	playNoviz=false;
		  	simplayFlag=false;
		  //	setTimeout(playBase,getDelay(iteration));
		  	playBase();
...
function genSimChartsBase()
{
	...
	if(resetFlag==false)
	{
				...
				if(simplayFlag==true && pauseFlag==false)
					//setTimeout(simulateplayBase,getDelay(playIter));
					simulateplayBase();
	}
	else
		reset();
...
function genChartsBase(output)
{
	//var jsonData = JSON.parse(output);
	
	//console.dir(output);
	isContinue =  output.continued;
	console.log("in base: ",iteration,"is continued:",isContinue);
	if(isContinue==0)
	{
		if(simplayFlag==false)
		  {
	...
	else if(resetFlag==false)
	{
		...
		if(estimate.length>0)
		{
			...
			{
				var quadObj = {"xb":estimate[i].xb,"xe":estimate[i].xe,"yb":estimate[i].yb,"ye":estimate[i].ye,"a":estimate[i].a};
				segmentChain[iteration][i] = quadObj;
				segmentArr[i] = quadObj;
			}
				
			//console.log(segmentChain[iteration]);
