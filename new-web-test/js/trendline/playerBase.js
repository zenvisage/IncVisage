...
function playBase(){
	...
	if(resetFlag==false)
	{
		...
		if(playFlag==true && isContinue==1)//(iteration<=xMax && playFlag==true)
		{
...
function genChartsBase(output)
{
	...
	console.log("in base: ",iteration);

	var isContinue = output.continued;
	...
	if(resetFlag==false)	 
	{
		...
		if(dataset == "sensordata")
		{
			...
			yMin[iteration] = yRange.aMin;
			yMax[iteration] = yRange.aMax;
		...
		else
		{
			...
			//{
				...
				currAvgMax = yRange.aMax+2;
				currAvgMin=yRange.aMin-2;
			//}
		...
		{
			var segObj = {"b":segment[i].b,"e":segment[i].e,"a":segment[i].a};
			segmentArr[i] = segObj;
			segmentChain[iteration][i] = segObj;
			//console.log(segmentArr[i]);
		}
	  ...
	  if(iteration>=xMax)
	  {
		  ...
		  else
		  {
	  		...
	  		if(simplayFlag==true)
			{
				...
				document.getElementById("forward").style.opacity = "0.1";
				document.getElementById("back").style.opacity = "0.1";
				console.log(playIter,"------pasue clicked-----",iteration,"-----simplay but still playBase-----",pauseIter);
				simulateplayBase();
			}
			else
			{
