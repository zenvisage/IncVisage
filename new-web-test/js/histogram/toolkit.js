...
function checkAnswer(){
  ...
  if(pauseIter==0)
    maxPause=xMax;
  if(currentDataset == "sensordata")
  {
    if(pauseIter==xMin-1)
      maxPause=xMax;
    else
      maxPause =  xMin+pauseIter-1;

    if(qType[questionNo].toString() == "N")
    {
      console.log(1*xMax-maxPause,maxPause,xMax);
      scoreV1 = (1.0*xMax-maxPause)/(38)*accuracyV1*(100.0);
      scoreQ = scoreV1;
      TotalScore += scoreV1;
    ...
    {
      scoreV3 = (1.0*xMax-maxPause)/(38)*accuracyV3*(100.0);
      scoreQ = scoreV3;
      TotalScore += scoreV3;
  ...
  else
  {
    ...
    document.getElementById('questionText').innerHTML = "You recived "+scoreQ.toFixed(2)+" pts.</br></br></br>"+question[questionNo];
