...
public final class RoaringArray implements Cloneable, Externalizable {
    ...
    protected RoaringArray() {
        ...
        this.keyIndexMapping= new HashMap<Integer, Integer>();
    }

    protected void append(short key, Container value) {
        extendArray(1);
        this.array[this.size++] = new Element(key, value);
        this.keyIndexMapping.put((int) key, this.size-1);
    }
    ...
    protected void appendCopy(RoaringArray sa, int index) {
        ...
        this.keyIndexMapping.put((int) sa.array[index].key, this.size-1);
    }
    ...
    protected void appendCopy(RoaringArray sa, int startingIndex, int end) {
        ...
        for (int i = startingIndex; i < end; ++i) {
            ...
            this.keyIndexMapping.put((int) sa.array[i].key, this.size-1);
        }
    ...
    protected void appendCopiesUntil(RoaringArray sourceArray, short stoppingKey) {
        ...
        for (int i = 0; i < sourceArray.size; ++i) {
            ...
            this.keyIndexMapping.put((int) sourceArray.array[i].key, this.size-1);
        }
    }
    ...
    public RoaringArray clone() throws CloneNotSupportedException {
        ...
        sa.keyIndexMapping.putAll(this.keyIndexMapping);
 ...
 //   	int i = this.keyIndexMapping.get(x);
        if (i < 0)
            return null;
    ...
    protected int getNextIndex(short x) {
        // before the binary search, we optimize for frequent cases
        if ((size == 0) || (array[size - 1].key == x))
            return 0;
        // no luck we have to go through the list
        return this.binarySearchforNextIndex(0, size, x);
    }
    ...
    private int binarySearchforNextIndex(int begin, int end, short key) {
        int low = begin;
        int high = end - 1;
        int ikey = Util.toIntUnsigned(key);
    ...
    Element[] array = null;
    Map<Integer,Integer> keyIndexMapping =null;
