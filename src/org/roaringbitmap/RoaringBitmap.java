...
public class RoaringBitmap implements Cloneable, Serializable, Iterable<Integer>, Externalizable, ImmutableBitmapDataProvider {
	...
	public static SampledResult sampledAndWithoutReplacement(final RoaringBitmap x1,final RoaringBitmap x2,int sample) {    	
		short previousKey = getRandomContainerKey(x1);
		return sampledAndWithoutReplacement(x1,x2,sample,previousKey);
	} 

	public static SampledResult sampledAndThreeWithoutReplacement(final RoaringBitmap x1,final RoaringBitmap x2,final RoaringBitmap x3,int sample) {    	
		short previousKey = getRandomContainerKey(x1);
		return sampledAndThreeWithoutReplacement(x1,x2,x3,sample,previousKey);
	}
	
	public static SampledResult sampledAndWithoutReplacement(final RoaringBitmap x1,
			final RoaringBitmap x2,int sample,short previousKey) {
	
		ArrayList<SampledContainer> sampledContainers = new ArrayList<SampledContainer>();
		SampledResult sampledResult = new SampledResult();
		final int length1 = x1.highLowContainer.size(), length2 = x2.highLowContainer.size();
		int previouspos= x1.highLowContainer.getIndex(previousKey);
		int pos1=x1.highLowContainer.getNextIndex(previousKey);
		int pos2=x2.highLowContainer.getNextIndex(previousKey);
		int startpos1=pos1;
		int startpos2=pos2;
		Integer a;
        int count=0;
        short lastindex=0;
 //       System.out.println("Previous Position- Pos1:"+previouspos);
        
    	while (count < sample & pos1 < length1 && pos2 < length2) {
			final short s1 = x1.highLowContainer.getKeyAtIndex(pos1);
			final short s2 = x2.highLowContainer.getKeyAtIndex(pos2);
//			System.out.println("Current Positions- Pos1:"+pos1+"- Pos2:"+pos2);
			lastindex=s1;
//			 System.out.println(" Pos1:"+pos1);
			if (s1 == s2) {
//				System.out.println("Matched Positions- Pos1:"+pos1+"- Pos2:"+pos2);
				final Container c1 = x1.highLowContainer.getContainerAtIndex(pos1);
				final Container c2 = x2.highLowContainer.getContainerAtIndex(pos2);
				final Container c = c1.and(c2);
				int card=c.getCardinality();
				if(card>0){
				SampledContainer sampledContainer = new SampledContainer();
				sampledContainer.index=s1;
				sampledContainer.container=c;
				count=count+card;		
				a = sampledResult.counts.get(c);
				if(a==null){
					sampledContainers.add(sampledContainer);
					a=1;
				}				
				else
					a=a+1;
				sampledResult.counts.put((int)s1, a);
				}
				++pos1;
				++pos2;
			} else if (s1 < s2) {
...
//					System.out.println("Matched Positions- Pos1:"+pos1+"- Pos2:"+pos2);
		...
		sampledResult.sampledContainers=sampledContainers;
		sampledResult.lastIndex=lastindex;
	...
	public static SampledResult sampledAndThreeWithoutReplacement(final RoaringBitmap x1,
			final RoaringBitmap x2,final RoaringBitmap x3,int sample,short previousKey) {
	
		ArrayList<SampledContainer> sampledContainers = new ArrayList<SampledContainer>();
		SampledResult sampledResult = new SampledResult();
		final int length1 = x1.highLowContainer.size(), length2 = x2.highLowContainer.size(), length3 = x3.highLowContainer.size();
		int previouspos= x1.highLowContainer.getIndex(previousKey);
		int pos1=x1.highLowContainer.getNextIndex(previousKey);
		int pos2=x2.highLowContainer.getNextIndex(previousKey);
		int pos3=x3.highLowContainer.getNextIndex(previousKey);
		int startpos1=pos1;
		int startpos2=pos2;
		int startpos3=pos3;
		Integer a;
 ...
 //       System.out.println("Previous Position- Pos1:"+previouspos);
    	...
    	while (count < sample & pos1 < length1 && pos2 < length2) {
			final short s1 = x1.highLowContainer.getKeyAtIndex(pos1);
			final short s2 = x2.highLowContainer.getKeyAtIndex(pos2);
			final short s3 = x3.highLowContainer.getKeyAtIndex(pos3);
//			System.out.println("Current Positions- Pos1:"+pos1+"- Pos2:"+pos2);
			lastindex=s1;
//			 System.out.println(" Pos1:"+pos1);
			if (s1 == s2 && s1 == s3) {
//				System.out.println("Matched Positions- Pos1:"+pos1+"- Pos2:"+pos2);
				...
				if(s2 > s3)
					...
					pos1 = x1.highLowContainer.advanceUntil(s3,pos1);
			} else if(s2 < s1 && s2 < s3){ 
				if(s1 > s3)
					pos2 = x2.highLowContainer.advanceUntil(s1,pos2);
				else
					pos2 = x2.highLowContainer.advanceUntil(s3,pos2);
			}else{
				if(s1 > s2)
					pos3 = x3.highLowContainer.advanceUntil(s1,pos3);
				else
					pos3 = x3.highLowContainer.advanceUntil(s2,pos3);
			}
		}
		...
		while (count < sample && pos1 < length1 && pos2 < length2 && pos3 < length3) {
	//		  System.out.println("Current Positions- Pos1:"+pos1+"- Pos2:"+pos2);
		  		final short s1 = x1.highLowContainer.getKeyAtIndex(pos1);
				final short s2 = x2.highLowContainer.getKeyAtIndex(pos2);
				final short s3 = x3.highLowContainer.getKeyAtIndex(pos3);
...
//					System.out.println("Matched Positions- Pos1:"+pos1+"- Pos2:"+pos2);
					final Container c1 = x1.highLowContainer.getContainerAtIndex(pos1);
					final Container c2 = x2.highLowContainer.getContainerAtIndex(pos2);
					final Container c3 = x3.highLowContainer.getContainerAtIndex(pos2);
					final Container c = c1.and(c2.and(c3));
					int card=c.getCardinality();
					if(card>0){
					SampledContainer sampledContainer = new SampledContainer();
					sampledContainer.index=s1;
	...
	public static SampledResult sampledAnd(final RoaringBitmap x1,
			final RoaringBitmap x2, int samples, int n) {

		//  	System.out.println("Number of Container in Rbitmap1:"+length1);
		//  	System.out.println("Number of Container in Rbitmap1:"+length2);
		ArrayList<SampledContainer> sampledContainers = new ArrayList<SampledContainer>();
		SampledResult sampledResult = new SampledResult();
		int count=0;
		int iter=0;
		...
		int progresscount=0;
		while(count < samples && (iter < 50 && progresscount==0) ){ 	

			int randomInt = randomGenerator.nextInt(n);
			short highbits = Util.highbits(randomInt);    	 
			//	Integer pos1=x1.highLowContainer.keyIndexMapping.get(highbits);
			//	Integer pos2=x2.highLowContainer.keyIndexMapping.get(highbits);

			Integer pos1=x1.highLowContainer.getIndex(highbits);
			Integer pos2=x2.highLowContainer.getIndex(highbits);
			Integer a=0;
			if ( pos1 > -1 && pos2 > -1 ) {
				final Container c1 = x1.highLowContainer.getContainerAtIndex(pos1);
				final Container c2 = x2.highLowContainer.getContainerAtIndex(pos2);
				...
				progresscount=0;
				SampledContainer sampledContainer = new SampledContainer();
				sampledContainer.index=highbits;
				sampledContainer.container=c;
				count=count+card;		
				a=sampledResult.counts.get(highbits);
				if(a==null){
					sampledContainers.add(sampledContainer);
					a=1;
				}				
					...
					a=a+1;
				sampledResult.counts.put((int) highbits, a);
			} 
	...
	public static SampledResult sampledAndThree(final RoaringBitmap x1,
			final RoaringBitmap x2, final RoaringBitmap x3, int samples, int n) {
		...
		while(count < samples && (iter < 50 && progresscount==0) ){ 	
			...
			Integer pos1=x1.highLowContainer.getIndex(highbits);
			Integer pos2=x2.highLowContainer.getIndex(highbits);
			Integer pos3=x3.highLowContainer.getIndex(highbits);
			Integer a=0;
			if ( pos1 > -1 && pos2 > -1 && pos3 > -1) {
				final Container c1 = x1.highLowContainer.getContainerAtIndex(pos1);
				final Container c2 = x2.highLowContainer.getContainerAtIndex(pos2);
				final Container c3 = x3.highLowContainer.getContainerAtIndex(pos3);
				final Container c = c1.and(c2.and(c3)); 
				int card=c.getCardinality();
				if (card==0){
					progresscount=progresscount+1;
	...
	public static SampledResult sampledOR(final RoaringBitmap x1,
			final RoaringBitmap x2, int samples, int n) {
		final RoaringBitmap answer = new RoaringBitmap();
		//  	System.out.println("Number of Container in Rbitmap1:"+length1);
		//  	System.out.println("Number of Container in Rbitmap1:"+length2);
	...
	public static SampledResult sampledORWithoutReplacement(final RoaringBitmap x1,final RoaringBitmap x2,int sample) {    	
		short previousKey = getRandomContainerKey(x1);
		return sampledORWithoutReplacement(x1,x2,sample,previousKey);
	} 

	public static SampledResult sampledORWithoutReplacement(final RoaringBitmap x1,
			final RoaringBitmap x2,int sample,short previousKey) {
		...
		while (count < sample & pos1 < startpos1 && pos2 < startpos2) {
				final short s1 = x1.highLowContainer.getKeyAtIndex(pos1);
				final short s2 = x2.highLowContainer.getKeyAtIndex(pos2);
				if (s1 == s2) {
					final Container c1 = x1.highLowContainer.getContainerAtIndex(pos1);
	...
	public static SampledResult sampleBits(final RoaringBitmap x1,int samples, int n) {
		ArrayList<SampledContainer> sampledContainers = new ArrayList<SampledContainer>();
		SampledResult sampledResult = new SampledResult();
		int count=0;
		Random randomGenerator = new Random();
		int numberofConaitners=x1.highLowContainer.size;
		Integer a=0;
		while(count < samples){    	
			int pos1 = randomGenerator.nextInt(numberofConaitners);
			final Container c1 = x1.highLowContainer.getContainerAtIndex(pos1);
			count=count+c1.getCardinality();
			SampledContainer sampledContainer = new SampledContainer();
			sampledContainer.index= x1.highLowContainer.getKeyAtIndex(pos1) ;
			sampledContainer.container=c1;
			a=sampledResult.counts.get((int)sampledContainer.index);
			if(a==null){
				sampledContainers.add(sampledContainer);
				a=1;
			}				
				...
				a=a+1;
			sampledResult.counts.put((int) sampledContainer.index, a);	
			sampledResult.totalContainers=sampledResult.totalContainers+1;
	...
	public static short getRandomContainerKey(final RoaringBitmap x1){
		int numberofConaitners=x1.highLowContainer.size;
		Random randomGenerator = new Random();
		int pos1 = randomGenerator.nextInt(numberofConaitners);
		return x1.highLowContainer.getKeyAtIndex(pos1);
		//return x1.highLowContainer.getKeyAtIndex(0);
	}


	public static SampledResult sampleBitsWithoutReplacement(final RoaringBitmap x1,int samples, int n) {    	
		short previousKey = getRandomContainerKey(x1);
		return sampleBitsWithoutReplacement(x1,samples,n,previousKey);
	} 


	public static SampledResult sampleBitsWithoutReplacement(final RoaringBitmap x1,int samples, int n,short previousKey) {
		ArrayList<SampledContainer> sampledContainers = new ArrayList<SampledContainer>();
		SampledResult sampledResult = new SampledResult();
		int count=0;
		int numberofConaitners=x1.highLowContainer.size;
		...
		int previousPos= x1.highLowContainer.getIndex(previousKey);
		int pos1= (previousPos+1)%numberofConaitners;
	
		Integer a=0;
		short lastIndex=0;
	//	System.out.println("Previous Position:"+previousPos);
		while(count < samples && pos1!=previousPos){    	
			final Container c1 = x1.highLowContainer.getContainerAtIndex(pos1);
	//		System.out.println("Current Position:"+pos1);
			count=count+c1.getCardinality();
			SampledContainer sampledContainer = new SampledContainer();
			sampledContainer.index= x1.highLowContainer.getKeyAtIndex(pos1) ;
			sampledContainer.container=c1;
			...
			if(a==null){
				...
				a=a+1;
			sampledResult.counts.put((int) sampledContainer.index, a);	
			lastIndex=sampledContainer.index;
			sampledResult.totalContainers=sampledResult.totalContainers+1;
			pos1=(pos1+1)%numberofConaitners;
